...
TexturedVertex Background::vertices[4] = {
        TexturedVertex{
                {-1,  1, -0.01f},
                {0.f, 1.f}
        },
...
uint16_t Background::indices[6] = {0, 3, 1, 1, 3, 2};
...
void Background::set_pos(const vec2 &pos) {
    position_ = {pos.x, pos.y};
}

void Background::set_pos(float x, float y) {
    position_ = {x, y};
}
...
void Background::set_color(const vec3 &color) {
