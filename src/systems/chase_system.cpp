...
void ChaseSystem::update(Blackboard &blackboard, entt::DefaultRegistry& registry) {
    ...
    auto dracula_chaser_view = registry.view<Dracula, Chases, Transform, Velocity, Interactable>();
    ...
    for (auto entity: dracula_chaser_view) {
        auto &dracula = dracula_chaser_view.get<Dracula>(entity);
        auto &transform = dracula_chaser_view.get<Transform>(entity);
        auto &velocity = dracula_chaser_view.get<Velocity>(entity);
        auto &chases = dracula_chaser_view.get<Chases>(entity);
        auto &interactable = dracula_chaser_view.get<Interactable>(entity);
        ...
        if(chases.evading&&dracula.alive){
            if (chasedPosition.x < transform.x) {
                velocity.x_velocity = chases.chase_speed*5;
            } else if (chasedPosition.x > transform.x) {
                velocity.x_velocity = -chases.chase_speed*5;
